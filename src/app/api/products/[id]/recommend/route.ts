...
export async function GET(req: Request, { params }: Context) {
  try {
    ...
    const orderIds = soldItems.map(i => i.orderId);

    // 2. If no orders exist, fallback to category siblings
    if (orderIds.length === 0) {
      if (!product.categoryId) return NextResponse.json({ ok: true, recommendations: [] });
    ...
    const sortedRaw = Array.from(freqMap.values()).sort((a, b) => b.count - a.count).map(entry => entry.product).slice(0, 4);

    // 5. Pad with category siblings if needed
    let finalRecs = sortedRaw;
    if (finalRecs.length < 4 && product.categoryId) {
      const existingIds = [product.id, ...finalRecs.map((r: any) => r.id)];
      const fillers = await prisma.product.findMany({
        where: { categoryId: product.categoryId, id: { notIn: existingIds }, active: true },
